...
class SimplePageRank(object):
    ...
    def update_weights(nodes, num_nodes):
        ...
        def distribute_weights((node, (weight, targets))):
            RANDOM_FACTOR = 0.1
            STAY = 0.05
            FOLLOW_LINK = 1 - RANDOM_FACTOR - STAY
            nodelist = []

            #Weight for STAY, RANDOMFACTOR
            nodelist.append( (node, (STAY*weight+RANDOM_FACTOR, list(targets)) ))
            ...
            if length == 0:
                ...
                for target in range(num_nodes):
                    if target != node:
                        nodelist.append( (target, (linkWeight, [])) )
            else:
        ...
        def collect_weights((node, values)):
            ...
            return (node,(weight, frozenset(targets)) )
