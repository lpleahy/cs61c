...
int pass_one(FILE* input, FILE* output, SymbolTable* symtbl) {
	...
	while(fgets(buf, BUF_SIZE, input)) {
		...
		char label[BUF_SIZE];
		int label_assigned = 0;
		if (pointer) {
			label_assigned = 1;
			strcpy(label, buf);
			char* temp_pointer = strchr(label, ':');
			*(temp_pointer + 1) = '\0';
			if((add_if_label(input_line, label, byte_offset, symtbl)) != 1) ret_code = -1;
      ...
      temp++;
		...
		if (num_args == 0) {
      int check;
			if (label_assigned) byte_offset += 4;
          if (token) {
               check = write_pass_one(output, token, args, num_args);
              if (!check) {
                  ret_code = -1;
...
int pass_two(FILE *input, FILE* output, SymbolTable* symtbl, SymbolTable* reltbl) {
    ...
    if (label) {
		...
		// raise_inst_error().
		if (translate_inst(output, token, args, (size_t) num_args, byte_offset, symtbl, reltbl)) {
			raise_inst_error(input_line, token, args, num_args);
			retval = -1;
		}
