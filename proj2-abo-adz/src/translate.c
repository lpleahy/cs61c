...
const int TWO_POW_TWENTYSIX = 67108864;  // 2^26
const int BUF_SIZE1 = 1024;
...
unsigned write_pass_one(FILE* output, const char* name, char** args, int num_args) {
    ...
    if (strcmp(name, "li") == 0) {
        ...
        char int_str[BUF_SIZE1];
        long int stor;
        if (translate_num(&stor, args[1], INT32_MIN, UINT32_MAX) == -1) return 0;

        if ((stor & 0xffff0000) == 0) { 
          // We can use addiu instead
          sprintf(int_str, "%ld", stor);
          ...
          if (!fprintf(output, "addiu %s %s %s\n", new_args[0], "$0", new_args[2])) return 0;

          return 1;
        } else {
          // We need to use lui and ori
          long int temp = (stor >> 16); // Top 16 bits
          sprintf(int_str, "%ld", temp);

          char* new_args[2];
          new_args[0] = args[0];
          new_args[1] = int_str;

          if (!fprintf(output, "lui %s %s\n", "$at", new_args[1])) return 0;
          temp = (stor & 0x0000ffff);
          sprintf(int_str, "%ld", temp);

          new_args[1] = int_str;
          if (!fprintf(output, "ori %s %s %s\n", new_args[0], "$at", new_args[1])) return 0;
        ...
        if (!fprintf(output, "add %s %s %s\n", new_args[0], new_args[1], new_args[2])) return 0;
        ...
        if (!fprintf(output, "slt %s %s %s\n", new_args[0], new_args[1], new_args[2])) return 0;

        new_args[1] = "$0";
        new_args[2] = args[2];
        if (!fprintf(output, "bne %s %s %s\n", new_args[0], new_args[1], new_args[2])) return 0;
        ...
        /* YOUR CODE HERE */
        if (num_args != 3) return 0;
        if (!fprintf(output, "add %s %s %s\n", "$at" ,args[1], args[2])) return 0;
        if (!fprintf(output, "add %s %s %s\n", args[0], args[0], "$at")) return 0;
        return 2;       
    } else if (strcmp(name, "swpr") == 0) {
        /* YOUR CODE HERE */
        if (num_args != 2) return 0;
        if (!fprintf(output, "add %s %s %s\n", "$at", args[0], "$zero")) return 0;
        if (!fprintf(output, "add %s %s %s\n", args[0], "$zero", args[1])) return 0;
        if (!fprintf(output, "add %s %s %s\n", args[1], "$zerg", "$at")) return 0;
        return 2;       
    } else if (strcmp(name, "mul") == 0) {
        /* YOUR CODE HERE */
        if (num_args != 3) return 0;
        if (!fprintf(output, "mult %s %s\n", args[1], args[2])) return 0;
        if (!fprintf(output, "mflo %s\n", args[0])) return 0;
        return 2;       
    } else if (strcmp(name, "div") == 0) {
        /* YOUR CODE HERE */
        if (num_args != 3) return 0;
        if (!fprintf(output, "div %s %s\n", args[1], args[2])) return 0;
        if (!fprintf(output, "mflo %s\n", args[0])) return 0;
        return 2;       
...
/* Writes the instruction in hexadecimal format to OUTPUT during pass #2.
   ...
   Some function declarations for the write_*() functions are provided in 
   translate.h, and you MUST implement these and use these as function 
   headers. You may use helper functions, but you still must implement
...
int write_rtype(uint8_t funct, FILE* output, char** args, size_t num_args) {
    ...
    if (rt == -1 || rs == -1 || rd == -1) return -1;
    uint32_t instruction = 0;
    // opcode is 0 forall R-type instructions
    instruction = instruction | (rs << 21) | (rt << 16) | (rd << 11) | funct;
...
int write_shift(uint8_t funct, FILE* output, char** args, size_t num_args) {
  ...
  uint32_t instruction = 0;
  instruction = instruction | (rt << 16) | (rd << 11) | ((uint32_t) shamt << 6) |((uint32_t) funct);
  write_inst_hex(output, instruction);
  return 0;
}
...
int write_jr(uint8_t funct, FILE* output, char** args, size_t num_args) {
  ...
  uint32_t instruction = 0;
  instruction = instruction | (rs << 21) | 8; // funct
  write_inst_hex(output, instruction);
  return 0;
}
...
int write_addiu(uint8_t opcode, FILE* output, char** args, size_t num_args) {
  ...
  uint32_t instruction = 0;
  instruction = instruction | (opcode << 26) | (rs << 21) | (rt << 16) | ((uint16_t) imm);
  write_inst_hex(output, instruction);
  return 0;
}
...
int write_lui(uint8_t opcode, FILE* output, char** args, size_t num_args) {
  ...
  uint32_t instruction = 0;
  instruction = instruction | (opcode << 26) | (rt << 16) | ((uint16_t) imm);
  write_inst_hex(output, instruction);
  return 0;
}
...
int write_branch(uint8_t opcode, FILE* output, char** args, size_t num_args, uint32_t addr, SymbolTable* symtbl) {
  ...
  if (rs == -1 || rt == -1 || label_addr == -1) return -1;
  ...
  uint32_t instruction = 0;
  instruction = instruction | (opcode << 26) | (rs << 21) | (rt << 16) | ((uint16_t) offset);
  write_inst_hex(output, instruction);        
  return 0;
}
...
int write_jump(uint8_t opcode, FILE* output, char** args, size_t num_args, uint32_t addr, SymbolTable* reltbl) {
  ...
  if (label_addr == -1) add_to_table(reltbl, args[0], addr);
  uint32_t instruction = 0;
