...
static int sum_vectorized(int n, int *a)
{
    ...
    int final_sum = 0;
    for (i = 0; i < n/4*4; i+=4){
    	__m128i array = _mm_loadu_si128((__m128i*) (a+i));
    	sum = _mm_add_epi32(sum, array);
    }
    ...
    return final_sum + sum_array[0] + sum_array[1] + sum_array[2] + sum_array[3];
}
...
static int sum_vectorized_unrolled(int n, int *a)
{
    ...
    for (i = 0; i < n/16*16; i+=16){
        __m128i array0 = _mm_loadu_si128((__m128i*) (a+i));
        __m128i array1 = _mm_loadu_si128((__m128i*) (a+i+4));
        __m128i array2 = _mm_loadu_si128((__m128i*) (a+i+8));
        __m128i array3 = _mm_loadu_si128((__m128i*) (a+i+12));
        sum = _mm_add_epi32(sum, array0);
        sum = _mm_add_epi32(sum, array1);
        sum = _mm_add_epi32(sum, array2);
        sum = _mm_add_epi32(sum, array3);
    }
